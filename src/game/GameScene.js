class GameScene extends Phaser.Scene {
  ...
  create() {
    this._gui = this.add.container(0, 0).setScrollFactor(0).setDepth(200).setVisible(false);

const dim = this.add.rectangle(gameWidth / 2, gameHeight / 2, gameWidth, gameHeight, 0x000000, 0.5).setInteractive();
const panel = this.add.rectangle(gameWidth / 2, gameHeight / 2, 520, 300, 0x1b1b1b, 1).setStrokeStyle(3, 0xffffff, 0.2);
const title = this.add.text(gameWidth / 2, gameHeight / 2 - 90, "My GUI", { fontSize: "36px", color: "#ffffff" }).setOrigin(0.5);

const close = this.add.text(gameWidth / 2 + 220, gameHeight / 2 - 130, "X", { fontSize: "28px", color: "#ff6666" })
  .setInteractive({ useHandCursor: true });

close.on("pointerup", () => this._gui.setVisible(false));
dim.on("pointerup", () => this._gui.setVisible(false));

this._gui.add([dim, panel, title, close]);

    setSceneRenderZoom(this);
    const menuContainer = this.add.container(0, 0).setScrollFactor(0).setDepth(30);
    this._menuContainer = menuContainer;
    this._menuContainer.setVisible(false);
    this._guiOpen = false;
    this._bgSpeedX = 0.1;
    this._bgSpeedY = 0.1;
    ...
    if (typeof window !== "undefined" && window.__gdFunModToggles) {
      this._funModToggles = {
        ...this._funModToggles,
        ...window.__gdFunModToggles
      };
    }
    this._applyFunModTogglesToSystems = () => {
      this._rawinbow.enabled = !!this._funModToggles.rainbow;
      this._noclip.setEnabled(!!this._funModToggles.noclip);
      this._noclip.bindPlayer(this._player);
      this._platformer.enabled = !!this._funModToggles.platformerTest;
      this._platformer.reset();
    };
    this._applyFunModTogglesToSystems();
    this._onFunModToggles = e => {
      this._funModToggles = {
        ...this._funModToggles,
        ...(e && e.detail || {})
      };
    ...
    if (typeof window !== "undefined") {
      window.addEventListener("gd:fun-mod-toggles", this._onFunModToggles);
      this.events.once("shutdown", () => {
        window.removeEventListener("gd:fun-mod-toggles", this._onFunModToggles);
      });
    }
    this._emitModHook = (phase, path, payload = {}) => {
      if (typeof window === "undefined") {
        return;
      }
      const detail = {
      ...
      if (window.GDModAPI && typeof window.GDModAPI.emit === "function") {
        window.GDModAPI.emit(phase, path, detail);
      }
      window.dispatchEvent(new CustomEvent(`gdmod:${phase}:${path}`, {
        detail
      }));
    };
    this._modHookState = {
      wasDead: !!this._state.isDead,
      wasOnGround: !!this._state.onGround
    };
    ...
    for (let i = 0; i < bunttons.length; i++) {
      const store = MenuLinks[i];
      const btnScale = 1 / 1.5;
      const frame = String(store.key || "");
      const frameKey = frame.endsWith(".png") ? frame : frame + ".png";
      const img = this.add.image(0, 0, "GJ_yourshit", frameKey).setScrollFactor(0).setDepth(30).setScale(btnScale).setInteractive();
      this._makeBouncyButton(img, btnScale, () => {
        this._guiOpen = true;
        if (window.openOverlay) {
          window.openOverlay();
        } else {
          this._menuContainer.setVisible(true);
        }
      }, () => this._menuActive);
      this._downloadBtns.push(img);
    }
    ...
    this._emitModHook("after", "game.loaded", {
      sceneKey: "GameScene"
    });
    this._emitModHook("after", "scene.loaded", {
      sceneKey: "GameScene"
    });
  ...
  _pauseGame() {
    if (!this._paused && !this._menuActive && !this._slideIn && !this._state.isDead && !this._levelWon) {
      this._emitModHook("before", "game.pause", {
        paused: true
      });
      this._paused = true;
      ...
      this._emitModHook("after", "game.pause", {
        paused: true
      });
    }
  }
  _resumeGame() {
    if (this._paused) {
      this._emitModHook("before", "game.resume", {
        paused: false
      });
      ...
      this._emitModHook("after", "game.resume", {
        paused: false
      });
    }
  }
  ...
  _startGame() {
    ...
    this._emitModHook("before", "menu.play.clicked", {
    ...
    this._positionAttemptsLabel();
    this._emitModHook("after", "menu.play.clicked", {
      menuActive: this._menuActive
    });
    this._emitModHook("after", "game.start", {
      attempt: this._attempts
    });
    this._emitModHook("after", "attempt.count", {
      attempt: this._attempts
    });
  }
  _pushButton() {
    ...
    if (!this._slideIn && !this._state.isDead) {
      ...
      if (!this._state.isFlying && this._state.canJump) {
        this._emitModHook("before", "player.jump", {
          x: this._playerWorldX,
          y: this._state.y
        });
        this._player.updateJump(0);
        this._totalJumps++;
        this._emitModHook("after", "player.jump", {
          x: this._playerWorldX,
          y: this._state.y,
  ...
  _restartLevel() {
    this._emitModHook("before", "level.restart", {
    ...
    this._positionAttemptsLabel();
    this._emitModHook("after", "level.restart", {
      attempt: this._attempts
    });
    this._emitModHook("after", "attempt.count", {
      attempt: this._attempts
  ...
  update(_time, delta) {
    ...
    this._emitModHook("during", "game.update", {
      delta
    });
    if (this._modHookState) {
      ...
      if (onGroundNow && !this._modHookState.wasOnGround) {
        this._emitModHook("after", "player.landed", {
          x: this._playerWorldX,
          y: this._state.y
        });
      } else if (!onGroundNow && this._modHookState.wasOnGround) {
        this._emitModHook("after", "player.airborne", {
          x: this._playerWorldX,
          y: this._state.y
        });
      }
    ...
    if (this._funModToggles.rainbow) {
